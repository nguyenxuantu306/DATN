app.controller("category-ctrl", function($scope, $http) {
	...
	$scope.create = function() {
		...
		// Kiểm tra trùng lặp tên danh mục sản phẩm
		var isDuplicate = $scope.items.some(function(existingItem) {
			return existingItem.categoryname === item.categoryname;
		});
		...
		if (isDuplicate) {
			...
			Swal.fire({
				icon: 'warning',
				title: 'Lỗi!',
				text: 'Tên danh mục đã tồn tại. Vui lòng chọn tên khác.',
			});
		} else {
	...
	$scope.update = function() {
		...
		// Kiểm tra trùng lặp tên danh mục sản phẩm
		var isDuplicate = $scope.items.some(function(existingItem) {
			return existingItem.categoryname === item.categoryname && existingItem.categoryid !== item.categoryid;
		});
		...
		if (isDuplicate) {
			...
			Swal.fire({
				icon: 'error',
				title: 'Lỗi!',
				text: 'Tên danh mục sản phẩm đã tồn tại. Vui lòng chọn tên khác.',
			});
		} else {
