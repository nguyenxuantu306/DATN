...
//			return ResponseEntity.status(HttpStatus.NOT_FOUND).body("QR code not found in the image.");
//		} catch (Exception e) {
	...
	public ResponseEntity<?> captureImage(Model model, @RequestBody Map<String, String> requestData) {
		...
		try {
			...
			try (ByteArrayInputStream bis = new ByteArrayInputStream(imageBytes)) {
				...
				Log.info("Quét thành công qr mã vé :" + decodedText);
				System.out.println("6");
				try {
					Integer bookingid = Integer.parseInt(decodedText);
					Booking booking = bookingService.findById(bookingid);
//|| booking.getTourDateBooking().getTourdate().getTourdates() == new Date()
					if (booking.getStatusbooking().getStatusbookingid() == 2 || booking.getTourDateBooking().getTourdate().getTourdates().equals(new Date())) {

						StatusBooking statusBooking = new StatusBooking();
						statusBooking.setStatusbookingid(5);
						booking.setStatusbooking(statusBooking);
					...
					else if (booking.getStatusbooking().getStatusbookingid() == 5) {
						
						Date today = new Date();
						 LocalDateTime tourDate =  booking.getUsedate();
						if (today.equals(tourDate)) {
							// Ngày hôm nay
							BookingDTO bookingDTOs = modelMapper.map(booking, BookingDTO.class);
							LocalDateTime usedate = bookingDTOs.getUsedate();
							Log.info("Vé có mã :" + bookingDTOs.getBookingid() + " vừa được dùng hôm nay lúc :"
									+ usedate);
							return new ResponseEntity<>(bookingDTOs, HttpStatus.OK);
						} else {

							BookingDTO bookingDTOs = modelMapper.map(booking, BookingDTO.class);
							Log.info("Vé có mã :" + bookingDTOs.getBookingid() + " đã được dùng trước đó");
							return new ResponseEntity<>(bookingDTOs, HttpStatus.OK);
						}
					...
					e.printStackTrace();
					Log.warn("Khong Lay Duoc Booking tu database");
