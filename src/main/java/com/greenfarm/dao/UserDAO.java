...
public interface UserDAO extends JpaRepository<User, Integer> {
	...
	@Query("SELECT NEW ReportSP(u, SUM(CASE WHEN so.name = 'Giao hàng thành công' THEN od.totalprice ELSE 0 END), COUNT(o)) "
	...
	List<ReportSP> totalPurchaseByUser();

	@Query("SELECT NEW ReportSP(u, SUM(CASE WHEN sb.name = 'Đặt tour thành công' THEN b.Totalprice ELSE 0 END), COUNT(b)) "
	...
	List<ReportSP> BookingTotalPurchaseByUser();
